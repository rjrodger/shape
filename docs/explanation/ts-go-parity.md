...
- The builder set, the chainable-method set, and the string DSL
  grammar—including the coercions, the string formats, the isolation builders
...
- **Structs are a Go-only convenience.** A struct is read into the same
  ...
  accepts or produces changes; the differential harness never reaches a struct.
...
The Rust port ([`rs/`](../../rs/README.md)) is held to the same two gates:
every corpus row
(none skipped; `make test-rs` sets `SHAPE_RS_STRICT=1`, which turns a
skipped row into a failure) and every differential case. Two of the Go
divergences listed earlier do not apply to it:
